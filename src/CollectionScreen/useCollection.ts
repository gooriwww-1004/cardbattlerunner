...
export function useCollection(allCards: CardDefinition[]) {
  // 🔥 localStorage 대신 그냥 메모리 초기값
  const [data, setData] = useState<CollectionData>(() => ({ ...DEFAULT }));
  ...
  const toggleActiveDeck = useCallback((uid: string) => {
    setData(p => {
      ...
      if (p.activeDeck.length >= 12) return p;
  ...
  const combineCards = useCallback((uid1: string, uid2: string) => {
    setData(p => {
      ...
      if (!c1 || !c2) return p;
      if (c1.level !== c2.level) return p;
      if (c1.level >= 5) return p;
